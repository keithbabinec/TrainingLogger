CREATE PROCEDURE [dbo].[RemoveLiftingActivity]
(
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT	TOP 1 @MatchedRecordUserId = [dbo].[LiftingActivities].[UserObjectId]
		FROM	[dbo].[LiftingActivities]
		WHERE	[dbo].[LiftingActivities].[ID] = @ActivityId

		IF @MatchedRecordUserId IS NOT NULL
		BEGIN
			IF @MatchedRecordUserId = @UserId
			BEGIN
				DELETE FROM		[dbo].[LiftingActivities]
				WHERE			[dbo].[LiftingActivities].[ID] = @ActivityId
			END
			ELSE
			BEGIN
